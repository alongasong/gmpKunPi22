...
//    for (UShortInt dMM = 1; dMM <= 0xffff /*65535*//*maxUShortInt */ ; ++dMM) { //for110ii Nuz = 3; Nuz <= maxUnI; ++Nuz) {//for110ii
    for (unsigned long long dMM=1;dMM<=0xffffffff;++dMM) {
        

        //UnI
        tmpD01 = KunPi_4a1 * dMM;// (double)KunPiComplete* (double)dMM; //预算预期的分子,用乘法, 求(Pi/4),分子小于分母

            for (/*UShortInt*/ /*unsigned int*/ unsigned long long Nuz = tmpD01; Nuz <= (1 + tmpD01); ++Nuz) // dMM = (tmpD01 - 1); dMM <= (1 + tmpD01); ++dMM) {//for220jj
            {//for220jj
            //long double
            tmp_tryPi_4a = (long double)Nuz/ (double)dMM;//尝试 得出一个 （大概率）带误差的 tryPi值
            Deviat_real01 = fabs(/*KunPiComplete */ KunPi_4a1 -tmp_tryPi_4a); //算出 尝试tryPi与 realPi(KunPi)之间的 deviat_real
            ...
            //              12345678
            if (0 == (dMM % 10000000)) {  printf("."); /* cout << dMM; */ }
            //if (0xffff == dMM) break;
            if ( 0xffffffffu == dMM) break;
