...
int main()
{
    ...
    print16double_toStrPi(getKunPi(), 60); cout << endl;

    //
    

    long double Dev_nutz3denoM1 = fabs(1.0 * seekDeviationWuCha(3, 1, getKunPi()));

    long double tmp_tryPi;

    //---------------------------------
    long double KunPi01 = getKunPi();
    ...
    UnI tmpD01;
    int numI = 0;
    for (UnI Nuz= 3; Nuz<= maxUnI; ++Nuz) {//for110ii
        

        //UnI
            tmpD01 = (double)Nuz/ KunPi01;

        for (UnI dMM = (tmpD01 - 1); dMM <= (1 + tmpD01); ++dMM) {//for220jj
            //long double
            ...
            if ( Deviat_real01 <minDev)  {//if3300
                ...
                print16double_toStrPi((double(Nuz) / dMM), 60);
