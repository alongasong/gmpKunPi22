...
int main()
{
    ...
    for (UShortInt dMM = 1; dMM <= 0xffff /*65535*//*maxUShortInt */ ; ++dMM) { //for110ii Nuz = 3; Nuz <= maxUnI; ++Nuz) {//for110ii
        ...
        //UnI
            tmpD01 = (double)KunPiComplete *(double)dMM; //用乘法, 求(Pi/4),分子小于分母

            for (/*UShortInt*/ unsigned int Nuz = tmpD01/* - 1 */; Nuz <= (1 + tmpD01); ++Nuz) // dMM = (tmpD01 - 1); dMM <= (1 + tmpD01); ++dMM) {//for220jj
            {//for220jj
            //long double
